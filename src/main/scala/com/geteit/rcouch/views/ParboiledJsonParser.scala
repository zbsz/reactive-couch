 ...
 *
 * This implementation is borrowed from spray.json, and altered to use play json model.
 * It's used for async parsing of streamed ViewResponse, as it wasn't possible to use Jackson for this purpose (Jackson uses blocking IO).
 */
object ParboiledJsonParser extends Parser {
  ...
  def JsonNumber = rule { group(Integer ~ optional(Frac) ~ optional(Exp)) ~> ((str: String) => JsNumber(BigDecimal(str))) ~ WhiteSpace }
  ...
  def JsonTrue = rule { "true " ~ push(JsBoolean(true)) }

  def JsonFalse = rule { "false " ~ push(JsBoolean(false)) }
